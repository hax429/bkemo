...
This directory holds both the integration roadmap and feature-specific plans.
It is separate from `docs/agents/`, which describes current behavior. Source
code wins when any plan disagrees with the repository.
...
|---|---|---|---|
| P0 | [Platform-bound access tokens](./platform-bound-access-tokens.md) | One long-lived credential model for iOS/macOS/Obsidian/scripts with platform binding + misuse warnings | Confirmed 2026-08-06 |
...
| P1 | [Deterministic document parsing](./PARSING.md) | Turns attachments into editable Markdown without an AI dependency | Confirm source formats and result workflow |
| P1 | [URL bookmark enrichment](./url-bookmark-enrichment.md) | Paste URL → Notion-like card; save → Defuddle Markdown + Wayback | Locked 2026-08-08; v1 implemented |
...
5. **Add guarded Obsidian editing:** dock editing with portable IDs, conditional
   writes, and blocked conflict review. Explicit Append / Copy attachment only
   (no automatic vault projection in v1).
