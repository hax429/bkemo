...
**Status:** O4 in progress — private-release gate (no vault projection)

**Priority:** P1

**Confirmed:** 2026-08-01; v1 shape reconfirmed 2026-08-03 (no Save to vault /
Refresh / push; Copy attachment + Append + command palette in)

**Started:** 2026-08-01 — pairing/device credentials, gateway filters/audio/attachment
contracts, `/api/v1/obsidian/*`, and `out/obsidian/` scaffold landed.

**O1 (2026-08-03):** read sidebar — tag filter, attachment preview, cache +
change-cursor hydration after layout ready.

**O2 (2026-08-03):** capture/copy, offline outbox + IndexedDB audio, voice
transcription refresh, linear UI.

**O3 (2026-08-03):** guarded dock editing (double-click / blur auto-save /
conflict modal).

**O4 (2026-08-03):** hardening + command palette + Copy attachment + lifecycle;
disposable acceptance then production-origin private install (separately
approved). Vault projection remains out of v1.
...
Build a companion plugin under `out/obsidian/`. Version 1 is a native
...
The plugin has three deliberate data paths in v1:

1. Remote operations read and update bkemo directly.
2. Append inserts Markdown at the active editor cursor with a small bkemo source
   link. Appended fragments are not independently synchronized.
3. Explicit Copy attachment downloads a binary into
   `bkemo/attachments/<note-portable-id>/` (configurable root). Never automatic.

Save to vault / Refresh projection / push-from-vault are **out of v1**.
...
- dock editing of note content and inline hashtags;
- blur/Esc auto-save with visible dirty, saving, saved, and conflict states;
- typed capture and voice capture;
- refresh, Copy Markdown, Append to current note, Copy attachment, and Open in
  bkemo actions;
- command palette: Open bkemo list, Create new bkemo note, Append selected,
  Copy selected markdown, Copy attachment from selected.

The list and preview use native Obsidian controls and CSS variables. The plugin
registers commands and a ribbon action to reveal the view. Network activity and
...
### Save to vault — out of v1

Deferred past private release. Historical design (not shipped):

Save to vault would write under a configurable root that defaults to `bkemo/`.
It would use the Obsidian `Vault` and `FileManager` APIs and never write outside
the normalized root.
...
### Explicit push from vault — out of v1

Deferred with Save to vault. Not available in the private-release companion.

### Attachments

The sidebar previews remote attachments using authenticated requests. Append
does not download binaries automatically. An explicit Copy attachment action
stores a file under:
...
### O0: contract and disposable prototype — done

- Create a throwaway Obsidian vault.
- Scaffold from the official sample plugin under `out/obsidian/`.
...
### O2: capture and copy — done

- Add typed capture with idempotent creation.
- Add reviewed voice recording, upload, note creation, and asynchronous
  transcription refresh.
- Add Copy Markdown, Append to current note, and offline capture outbox.
- Add offline capture outbox and cache clearing.

### O3: guarded editing — done

- Add sidebar content editing with expected revisions (double-click → dock
  editor; blur/Esc auto-save; freeform `#tags` in Markdown).
- Comparison modal on `revision_conflict` (reload remote / keep editing; no
  force overwrite).

### O4: hardening and private release — in progress

- Command palette integrations + Copy attachment (explicit vault binary write).
- Rip projection persistence/wiring from v1; keep Append + Copy attachment.
- Lifecycle cleanup on unload; `isDesktopOnly: true`.
- Security helpers/tests: credential time validity, account-scoped ownership
  where clauses, revision-write match, attachment path under root, media limits.
- Disposable-vault acceptance checklist (see `docs/agents/OBSIDIAN.md`).
- Produce `main.js`, `manifest.json`, and `styles.css` for private installation.
- Install production-origin build in the real vault only after disposable
...
- Copy attachment stays inside the configured vault root.
- Inline hashtag edits round trip without a second tag model.
- Offline captures replay once and retain actionable failures.
- Command palette append/copy operate on the sidebar selection only.
