...
export class CodexSkillCatalog implements ProviderCommandCatalog {
  ...
  async listDropdownEntries(context: { includeBuiltIns: boolean }): Promise<ProviderCommandEntry[]> {
    ...
    const entries = skills.map((skill) => ({
      ...listedSkillToProviderEntry(skill, null),
      scope: skill.scope === 'repo' ? 'vault' as const : 'user' as const,
      isEditable: false,
      isDeletable: false,
...
export class CodexVaultSkillRepository implements ProviderVaultEntryRepository {
  constructor(
  ...
  async listVaultEntries(): Promise<ProviderCommandEntry[]> {
    ...
    for (const listedSkill of listedSkills) {
      ...
      entries.push({
        ...
        displayPrefix: '$',
        insertPrefix: '$',
        sourcePath: `${location.rootId === 'vault-agents' ? '.agents/skills' : '.codex/skills'}/${storedSkill.name}/SKILL.md`,
        persistenceKey: createCodexSkillPersistenceKey({
          rootId: location.rootId,
          currentName: location.name,
        }),
