...
export class ClaudianView extends ItemView {
  ...
  private buildHeader(header: HTMLElement): void {
    ...
    const titleText = titleEl.createEl('h4', { text: 'Codian', cls: 'claudian-title-text' });
    const host = this.plugin as FeatureHost & {
      onProductTitleDoubleClick?: (event: MouseEvent, anchor: HTMLElement) => void;
    };
    if (typeof host.onProductTitleDoubleClick === 'function') {
      titleText.addClass('como-mode-switch-target');
      titleText.addEventListener('dblclick', (event) => {
        event.preventDefault();
        event.stopPropagation();
        host.onProductTitleDoubleClick?.(event, titleText);
      });
    }
  }
