...
describe('MentionDropdownController', () => {
  ...
  describe('vault folder mentions', () => {
    ...
    it('inserts folder mention and reports a structured vault reference', () => {
      const onAttachFile = jest.fn();
      const onAttachVaultReference = jest.fn();
      const localCallbacks = createMockCallbacks({
        onAttachFile,
        onAttachVaultReference,
        getCachedVaultFolders: jest.fn().mockReturnValue([
          { name: 'src', path: 'src' },
        ]),
      });
      ...
      expect(onAttachVaultReference).toHaveBeenCalledWith('src', 'folder');
