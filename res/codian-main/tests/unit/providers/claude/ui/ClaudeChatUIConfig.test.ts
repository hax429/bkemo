...
describe('claudeChatUIConfig', () => {
  ...
  describe('getModelOptions', () => {
    ...
    it('adds one chat option for each enabled third-party service', () => {
      const options = claudeChatUIConfig.getModelOptions({
        providerConfigs: {
          claude: {
            thirdPartyServices: [{
              ...
              authMode: 'auth-token',
              secretId: 'codian-claude-bailian',
              ...
              lightweightModel: 'disabled-light',
              enabled: false,
            }],
          },
        },
      ...
      expect(options.some(option => option.value.includes('disabled'))).toBe(false);
    });
  });
