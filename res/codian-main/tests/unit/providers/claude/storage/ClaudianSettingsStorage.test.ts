...
describe('ClaudianSettingsStorage', () => {
  ...
  describe('load', () => {
    it('should return defaults when file does not exist', async () => {
      ...
      expect(result.enableLivePreviewComposer).toBe(true);
    ...
    it('removes the obsolete locale override from stored settings', async () => {
      mockAdapter.exists.mockResolvedValue(true);
      mockAdapter.read.mockResolvedValue(JSON.stringify({
        locale: 'zh-CN',
      }));
      ...
      expect(writtenContent).not.toHaveProperty('locale');
    });
