...
describe('prepareOpencodeLaunchArtifacts', () => {
  it('layers the managed prompt config on top of OPENCODE_CONFIG', async () => {
    ...
    expect(result.configPath).toBe(path.join(tmpRoot, '.codian', 'opencode', 'config.json'));
    expect(result.systemPromptPath).toBe(path.join(tmpRoot, '.codian', 'opencode', 'system.md'));
