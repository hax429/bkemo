...
describe('model selection namespacing', () => {
  describe('getProviderModelSelectionPrefix', () => {
    it('returns the registered prefix for each known provider', () => {
      ...
      expect(getProviderModelSelectionPrefix('kimi')).toBe('kimi-code/');
  ...
  describe('decodeProviderModelSelectionId', () => {
    it('decodes a namespaced id into its provider and model id', () => {
      ...
      expect(decodeProviderModelSelectionId('grok/default')).toEqual({
        providerId: 'grok',
        modelId: 'default',
      });
      expect(decodeProviderModelSelectionId('kimi-code/default')).toEqual({
        providerId: 'kimi',
        modelId: 'default',
      });
    });
