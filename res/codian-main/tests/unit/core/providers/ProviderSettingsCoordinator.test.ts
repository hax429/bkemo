...
describe('ProviderSettingsCoordinator', () => {
  ...
  describe('applyProviderEnablement', () => {
    it('refuses to disable the last enabled provider', () => {
      ...
      const result = ProviderSettingsCoordinator.applyProviderEnablement(
        settings,
        'claude',
        false,
      );

      expect(result).toBe('last-enabled-provider');
    ...
    it('disables Claude and falls back to another enabled provider', () => {
      const settings: Record<string, unknown> = {
        settingsProvider: 'claude',
        providerConfigs: {
          claude: { enabled: true },
