...
import type { Editor, PluginSettingTab, WorkspaceLeaf } from 'obsidian';
...
export default class ClaudianPlugin extends Plugin {
  ...
  async onload() {
    ...
    try {
      ...
      this.addRibbonIcon(this.getRibbonIconId(), this.getRibbonTooltip(), () => {
      ...
      const settingTab = this.createSettingTab();
      if (settingTab) {
        this.addSettingTab(settingTab);
      }
      this.scheduleRemainingSessionMetadataLoad();
      await this.onAfterCodianOnload();
    } finally {
      StartupProfiler.finishOnload();
    }
  }

  /** Override in host plugins to rename the ribbon tooltip. */
  protected getRibbonTooltip(): string {
    return 'Open codian';
  }

  /** Override in host plugins to use a stable leaf/ribbon icon id. */
  protected getRibbonIconId(): string {
    return CODIAN_RIBBON_ICON_ID;
  }

  /** Register icons needed before ribbon/views render. */
  protected registerPluginIcons(): void {
    addIcon(CODIAN_RIBBON_ICON_ID, CODIAN_RIBBON_ICON_SVG);
  }

  /** Stable icon for workspace leaf tabs; hosts may override. */
  getLeafIconId(): string {
    return this.getRibbonIconId();
  }

  /** Stable leaf tab label; hosts may override. */
  getLeafDisplayText(): string {
    return 'Codian';
  }

  /** Override in host plugins (e.g. como) to replace the Codian settings tab. */
  protected createSettingTab(): PluginSettingTab | null {
    return new ClaudianSettingTab(this.app, this);
  }

  /** Hook for host plugins to register extra views/commands after Codian onload. */
