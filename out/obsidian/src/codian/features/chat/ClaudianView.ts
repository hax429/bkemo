...
export class ClaudianView extends ItemView {
  ...
  getDisplayText(): string {
    const host = this.plugin as { getLeafDisplayText?: () => string };
    return host.getLeafDisplayText?.() ?? 'Codian';
  }

  getIcon(): string {
    const host = this.plugin as { getLeafIconId?: () => string };
    return host.getLeafIconId?.() ?? CODIAN_RIBBON_ICON_ID;
  }
  ...
  private buildHeader(header: HTMLElement): void {
    ...
    const titleText = titleEl.createEl('h4', { text: 'como', cls: 'claudian-title-text' });
    const host = this.plugin as FeatureHost & {
    ...
    if (typeof host.attachCodianModeSwitcher === 'function') {
      host.attachCodianModeSwitcher(titleText, titleEl, this.leaf);
      const kicker = host.getLeafModeKicker?.();
      if (kicker) {
        titleEl.createSpan({ cls: 'como-brand-kicker', text: kicker });
      }
    } else if (typeof host.onProductTitleDoubleClick === 'function') {
      titleText.addClass('como-mode-switch-target');
      titleText.addEventListener('dblclick', (event) => {
        event.preventDefault();
        event.stopPropagation();
        host.onProductTitleDoubleClick?.(event, titleText, this.leaf);
      });
    }
  }
