      ...
      for await (const chunk of agentService.query(preparedTurn, previousMessages, queryOptions)) {
        ...
        if (chunk.type === 'error' && chunk.code === 'provider_session_missing') {
          ...
          const notice = resolution === 'deleted'
            ? 'The provider session no longer exists. Its como record was removed; send again to start a new session.'
            : resolution === 'reset'
              ? 'The provider session no longer exists. como preserved the recoverable history; send again to rebuild the session.'
              : resolution === 'preserved'
                ? 'The provider session no longer exists. como preserved its record because the remaining history could not be verified.'
