...
export class MessageRenderer {
  ...
  appendInterruptIndicator(contentEl: HTMLElement): void {
    ...
    textEl.createSpan({
      ...
      text: '\u00B7 What should como do instead?',
    });
  }
  ...
  /**
   * Adds copy (+ optional save-as-note) actions to an assistant text block.
   */
  addTextCopyButton(textEl: HTMLElement, markdown: string): void {
    const actions = textEl.createDiv({ cls: 'claudian-text-actions' });

    const copyBtn = actions.createSpan({
      cls: 'claudian-text-copy-btn',
      attr: { 'aria-label': 'Copy markdown', title: 'Copy markdown' },
    });
    ...
    const host = this.plugin as FeatureHost & {
      saveChatMarkdownAsNote?: (markdown: string) => Promise<void>;
    };
    if (typeof host.saveChatMarkdownAsNote !== 'function') {
      return;
    }

    const saveBtn = actions.createSpan({
      cls: 'claudian-text-save-btn',
      attr: { 'aria-label': 'Save as note', title: 'Save as note' },
    });
    ...
    saveBtn.addEventListener('click', (e) => {
      ...
      runRendererAction(async () => {
        ...
        try {
          await host.saveChatMarkdownAsNote?.(content);
          saveBtn.empty();
          ...
          new Notice(error?.message || 'Could not save note');
        }
      });
    });
  }
