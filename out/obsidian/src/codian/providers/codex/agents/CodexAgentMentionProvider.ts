...
import {
  CODEX_AGENTS_PATH,
  type CodexSubagentStorage,
  parseCodexSubagentPersistenceKey,
} from '../storage/CodexSubagentStorage';
  ...
  searchAgents(query: string): Array<{
    ...
    return this.agents
      ...
      .map(a => ({
        ...
        filePath: `${CODEX_AGENTS_PATH}/${
          parseCodexSubagentPersistenceKey(a.persistenceKey)?.fileName ?? `${a.name}.toml`
        }`,
