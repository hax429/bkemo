...
import {
  OPENCODE_AGENT_PATH,
  type OpencodeAgentStorage,
  parseOpencodeAgentPersistenceKey,
} from '../storage/OpencodeAgentStorage';
  ...
  searchAgents(query: string): Array<{
    ...
    return this.agents
      ...
      .map((agent) => ({
        ...
        filePath: parseOpencodeAgentPersistenceKey(agent.persistenceKey)?.filePath
          ?? `${OPENCODE_AGENT_PATH}/${agent.name}.md`,
