...
type SettingsEntryKind = 'command' | 'skill';
...
export class SlashCommandModal extends Modal {
  ...
  constructor(
    ...
    private fixedKind?: SettingsEntryKind,
  ) {
    super(app);
  ...
  onOpen() {
    ...
    let selectedType: SettingsEntryKind = existingIsSkill ? 'skill' : (this.fixedKind ?? 'command');
    ...
    new Setting(contentEl)
      ...
      .addDropdown(dropdown => {
        dropdown
          ...
          .onChange(value => {
            selectedType = value as SettingsEntryKind;
            this.setTitle(this.existingEntry ? `Edit ${typeLabel()}` : `Add ${typeLabel()}`);
            updateSkillOnlyFields();
          });
        if (this.existingEntry || this.fixedKind) {
...
export class SlashCommandSettings {
  ...
  private allEntries: ProviderCommandEntry[] = [];
  ...
  constructor(
    ...
    private kind?: SettingsEntryKind,
  ) {
    this.app = app;
  ...
  private async loadAndRender(): Promise<void> {
    ...
    this.allEntries = await this.repository.listVaultEntries();
    this.commands = filterSettingsEntries(this.allEntries, this.kind);
    this.render();
  }
  ...
  private render(): void {
    ...
    const label = this.kind === 'skill'
      ? t('settings.settingsHub.workspaceSections.skills')
      : t('settings.settingsHub.workspaceSections.commands');
    headerEl.createSpan({ text: label, cls: 'claudian-sp-label' });
