...
describe('noteList helpers', () => {
  ...
  it('uses only cloud bkemo tags, not content hashtags', () => {
    assert.deepEqual(noteTags(note('Hello #inbox #later')), []);
    assert.deepEqual(
      noteTags(note('Hello #inbox', {
        tags: [{ portableId: '22222222-2222-2222-2222-222222222222', name: 'cloud' }],
      })),
      ['cloud'],
    );
  });
