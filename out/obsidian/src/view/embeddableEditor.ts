...
export type EmbeddableEditorOptions = {
  ...
  /** Focus left the editor chrome (not an internal focus move). */
  onBlur?: (value: string) => void;
  /** Return true to consume Escape. */
  onEscape?: (value: string) => boolean;
};
...
export function createEmbeddableMarkdownEditor(
      ...
      extensions.push(
        Prec.highest(
          keymap.of([
            ...
            {
              key: 'Escape',
              run: () => opts.onEscape?.(this.getValue()) ?? false,
            },
          ]),
        ),
      );
  ...
  cm.contentDOM.addEventListener('blur', () => {
    ...
    window.setTimeout(() => {
      ...
      if (active && editor.editorEl.contains(active)) return;
      opts.onBlur?.(editor.getValue());
    }, 0);
  });
