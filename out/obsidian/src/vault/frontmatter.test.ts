...
describe('projection frontmatter', () => {
  ...
  it('rejects path traversal outside the vault root', () => {
    assert.equal(assertPathUnderRoot('bkemo/a.md', 'bkemo'), 'bkemo/a.md');
    assert.throws(() => assertPathUnderRoot('../secret.md', 'bkemo'));
    assert.throws(() => assertPathUnderRoot('other/a.md', 'bkemo'));
  });

  it('round-trips markdown wrapping and stripping', () => {
    const wrapped = wrapProjectionMarkdown(buildProjectionFrontmatter(note, 'sha256:abc'), note.content);
    assert.match(wrapped, /^---\n/);
    assert.match(wrapped, /portableId: "67b2d411-221e-4dbe-98a4-d6db7c98c793"/);
    assert.equal(stripFrontmatter(wrapped).trim(), note.content);
  });
});
