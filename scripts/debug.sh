...
#
# Preferred data plane is Neon PostgreSQL + Cloudflare R2 (objectStorage=s3 in
# Settings → Storage). The launcher uses an attached Neon DATABASE_URL when the
# one-use development attach marker exists; otherwise it bootstraps a local
# Postgres (Docker or Homebrew postgresql@16) on :5433 so you can attach Neon
# and configure R2 from the UI.
#
...
# Prefer Neon + Cloudflare R2 after attach: set BKEMO_DEV_ALLOW_EXISTING_NEON=true,
# attach an existing Neon branch in Settings → Storage, then configure R2 there.
